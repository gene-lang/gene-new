...
Implementation coherence is based on visible implementations, not ownership/orphan restrictions. Any module may declare an implementation for any protocol and receiver type. At each use site, exactly one applicable visible implementation must exist; zero visible implementations is a missing-implementation error, and multiple applicable visible implementations is an ambiguity error. Import order must never resolve ambiguity. Generated implementations follow the same rule. MVP rejects overlapping generic implementations when they may both apply to the same concrete receiver type.
