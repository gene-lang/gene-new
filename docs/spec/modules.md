...
Namespace exposure and execution policy are separate controls: the sandbox
transaction and loader below select the first, and step, memory, and timeout
budgets bound the second.
...
- `($os/launch_dir)` returns the application's captured absolute launch
  directory as a Str. It differs from `this_pkg/root`, which may name an
  authenticated build snapshot.
...
- Runtime imports initialize a dependency once. Compile-time macro discovery
  does not execute dependency top-level forms.
...
- Preparation pauses worker module readers and restricts scheduler pumping to
  candidate-owned tasks. FFI, native type, and embedded web-module
  declarations are rejected.
- Before publishing a prepared generation, a host can call
  `$runtime/configure_module` to seal the execution policy across the
  generation's owned module closure. The numeric policy must match preparation.
  Once sealed, configuration is immutable (an identical configuration is
  idempotent). Escaped functions and typed methods retain the policy.
...
- `gene run [--package-root dir] file [--] [args...]` executes top level, then
  calls `main`. Positional strings form the first argument; everything after the
  entry file is ordinary program data.
...
1. `newNativeSyncCallback(callee, scope, signature)` validates and roots the
   callable, optionally wrapping it in an enforced `Callable` signature. No
   target/default code runs.
...
Callback invocation uses the normal callable/module boundary and enclosing
execution budget.
