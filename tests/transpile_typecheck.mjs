...
const sources = [
  ...
  join(root, "tests", "transpile", "web_foundations.gene"),
...
const generated = [
  ...
  join(work, "web_foundations.ts"),
  join(work, "web_foundations_api.ts"),
  join(work, "web_foundations_dep.ts"),
  join(work, "web_foundations.d.ts"),
  join(work, "web_foundations_api.d.ts"),
  join(work, "web_foundations_dep.d.ts"),
...
const contractPrefixes = ["callable.", "errors.", "self.", "nil_void.", "reader_wrap.", "generator."];
...
for (const item of contractCases) {
  ...
  mkdirSync(caseDir);
  const source = join(caseDir, name + ".gene");
  writeFileSync(source, item.web_source);
  for (const [file, contents] of Object.entries(item.web_files ?? {})) {
    writeFileSync(join(caseDir, file), contents);
  }
  execFileSync(gene, ["build", "--target", "web", "--out-dir", caseDir, source], { cwd: root, stdio: "pipe" });
  for (const file of readdirSync(caseDir)) {
    if (file.endsWith(".ts")) generated.push(join(caseDir, file));
  }
}
execFileSync(tsc, [
  "--noEmit",
