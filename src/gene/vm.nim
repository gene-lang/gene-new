...
proc buildBuiltins(app: Application): Scope =
  ...
  let urlError = newType("UrlError", NIL,
                         @[TypeField(name: "message", optional: false,
                                     typeExpr: newSym("Str"), scope: result)],
                         @[errorProtocol], result)
  result.define("UrlError", urlError)
  result.impls.add ProtocolImpl(protocol: errorProtocol,
                                receiver: urlError)
  let httpError = newType("HttpError", NIL,
                          @[TypeField(name: "message", optional: false,
                                      typeExpr: newSym("Str"), scope: result)],
                          @[errorProtocol], result)
  result.define("HttpError", httpError)
  result.impls.add ProtocolImpl(protocol: errorProtocol,
                                receiver: httpError)
  ...
  let stdStreamScope = newScope(result)
  ...
  let stdNodeScope = newScope(result)
  ...
  let stdParseScope = newScope(result)
  stdParseScope.define("parse_int", newNativeCallFn("parse_int", biParseInt,
                                                    acceptsNamed = false))
  stdParseScope.define("ParseError", result.vars["ParseError"])
  let stdScope = newScope(result)
  ...
  result.define("std", newNamespace("std", stdScope))
  let strScope = newScope(result)
  ...
  result.define("str", newNamespace("str", strScope))
  let htmlScope = newScope(result)
  ...
  result.define("html", newNamespace("html", htmlScope))
  let urlScope = newScope(result)
  ...
  urlScope.define("UrlError", result.vars["UrlError"])
  result.define("url", newNamespace("url", urlScope))
  let httpScope = newScope(result)
  let builtinsRoot = result
  ...
  let requestType = newType("Request", NIL,
                            @[strField("method"), strField("path"),
                              ...
                              TypeField(name: "params", optional: false,
                                        typeExpr: newSym("Map"), scope: result),
  ...
  httpScope.define("not_found", newNativeCallFn("http/not_found",
                                                biHttpNotFound,
                                                acceptsNamed = false))
  httpScope.define("HttpError", result.vars["HttpError"])
  let netLowerScope = newScope(result)
  netLowerScope.define("http", newNamespace("net/http", httpScope))
  result.define("net", newNamespace("net", netLowerScope))
