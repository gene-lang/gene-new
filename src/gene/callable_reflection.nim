...
## output. The VM supplies only a static type-name lookup.
...
proc safeAnnotation(expr: Value, scope: Scope, lookup: ReflectionTypeLookup,
  ...
  if expr.isTypeAlias:
    ...
    return safeAnnotation(expr.typeAliasExpr, scope, lookup, depth + 1)
  case expr.kind
  of vkType, vkProtocol: return expr
  of vkSymbol:
    if expr.symVal.endsWith("?") and expr.symVal.len > 1:
      let base = safeAnnotation(newSym(expr.symVal[0 .. ^2]), scope, lookup, depth + 1)
    ...
    if resolved.kind in {vkType, vkProtocol}:
      return safeAnnotation(resolved, scope, lookup, depth + 1)
  ...
  of vkNode:
    ...
    for item in expr.body:
      let safe = safeAnnotation(item, scope, lookup, depth + 1)
...
proc parameter(name, local: string, typ: Value, required, hasDefault: bool,
               scope: Scope, lookup: ReflectionTypeLookup): Value =
  var props = initPropTable()
...
proc describeCallable*(target: Value, lookup: ReflectionTypeLookup): Value =
  var props = initPropTable()
  ...
  of vkFunction:
    ...
    let proto = FunctionProto(target.fnCode)
    ...
    for p in proto.namedParams:
      named.add parameter(p.arg, p.local, p.typeExpr, not p.defaultValue.optional,
        p.defaultValue.defaultChunk != nil, scope, lookup)
    if proto.restParam.len > 0:
      rest = parameter(proto.restParam, proto.restParam, proto.restType, false, false,
                       scope, lookup)
    if proto.hasReturnType: resultType = safeAnnotation(proto.returnType, scope, lookup)
  ...
  of vkNativeFn, vkFfiCallable: props["category"] = newSym("native")
  of vkProtocolMessage: props["category"] = newSym("message")
  of vkType: props["category"] = newSym("type")
  else: discard
  if props["origin"].symbol("unknown"):
    return newNode(newSym("SignatureDescription"), props = props, immutable = true)
  ...
  for typ in errors:
    let safe = safeAnnotation(typ, scope, lookup)
