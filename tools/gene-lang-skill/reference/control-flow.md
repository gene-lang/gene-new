...
`$err`; `$err_msg` calls its retained `Error:message` implementation. Use
`catch Error` for any ordinary error. `fail` raises a recoverable value that
implements Error; `panic` does not. Clause heads align
