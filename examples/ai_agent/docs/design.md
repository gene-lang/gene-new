...
|---|---|---|---|---|
| 1 | ~~**Async subprocess**~~ — **closed (m8, cancellation extended in slice B/C2)**: `os/exec_async`/`exec_stream_async` run captured children on dedicated OS threads; stdout lines cross through a channel, the Task settles through the scheduler, and `cancel` (on `Task`) terminates the child and closes its stdout channel. `os/exec_stdio_async` uses the same settlement/cancellation boundary with inherited terminal streams for `/edit` and `/tty` | — | — | shipped in `src/gene/stdlib.nim` + spec tests |
